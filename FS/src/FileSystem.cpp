...
Inode* FileSystem::IAlloc()
{
	...
	if(sb->s_ninode <= 0)
	{
		...
		for(int i = 0; i < sb->s_isize; i++) //s_isize是格式化好的img中读取得到的
		{
			...
			for(int j = 0; j < FileSystem::INODE_NUMBER_PER_SECTOR; j++)
			{
				...
				int mode = *( p + j * sizeof(DiskInode)/sizeof(int) ); //对应了DIskInode的d_mode位置
...
{
	SuperBlock* sb;
	sb = this->GetFS();	/* 获取相应设备的SuperBlock内存副本 只有一个设备*/
